...
class CounterAddOperator : public AssociativeMergeOperator {
public:
  virtual bool Merge(
		     const Slice& key,
    ...
    unsigned long existing = 
      *(const unsigned long *)existing_value->data();
    unsigned long oper = 
      *(const unsigned long *)value.data();
    unsigned long result_value = existing + oper;
    *new_value = std::to_string(result_value);
    return true;        
  }

  virtual const char* Name() const override {
    return "CounterAddOperator";
  }
};
	      ...
	      rpc_cookie_t *cookie)
{
  ...
  if(rock->op == OP_PUT) {
    ...
    rocksdb::Slice key((const char *)&rock->key, 8);
    rocksdb::Slice value((const char *)&rock->value[0], value_sz);
    rocksdb::Status s = db->Merge(write_options, 
				  key,
				  value);
    if (!s.ok()){
      BOOST_LOG_TRIVIAL(fatal) << s.ToString();
...
void opendb(){
  ...
  options.merge_operator.reset(new rocksdb::CounterAddOperator);
